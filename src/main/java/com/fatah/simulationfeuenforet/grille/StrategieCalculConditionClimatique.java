...
public class StrategieCalculConditionClimatique implements StrategieCalcul{
    @Override
    public void calculer(List<int[]> pointsEnFeu, int[][] grille, List<int[]> listeIndicesFeu, double probabilite, boolean caseEnFeu,List<Integer> listeIndicesFeuAll,GrilleXY grilleXY,int indice,double humiditeSelectioner, double tomperatureSelectioner, double vitesseVentSelectioner) {
        ...
        for (int[] indices : pointsEnFeu) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (    TestePosition.positionValide(grille,positionCasesAdjacenteX, positionCasesAdjacenteY) &&
                        ...
                        ConditionClimatique.conditionsClilatiquePropagentFeu(direction,humiditeSelectioner,tomperatureSelectioner,
                                vitesseVentSelectioner,indice)>probabilite)
                {
